...
function allocatemail(id, data) {
  enrollment_data
    ...
    }</b>, New Course is assigned in your profile.Login into your Profile and check it </p>
    <ul>
...
adminRouter.delete("/deleteData", function (req, res) {
  ...
  //console.log(mail)
  enrollment_data.findOneAndDelete({ email_address: mail }).then(function () {
...
adminRouter.get("/table", (req, res) => {
  res.header("Access-Control-Allow-Origin", "*");
  res.header("Access-Control-Allow-Methods: GET, POST, PUT, DELETE");
  enrollment_data.find({ isApproved: true }).then(function (enrollments) {
